<?php
/**
 * Login, registration (OTP + Google), password reset, and profile updates.
 */
...
function handleAuthAction(string $action): void {
    ...
    switch ($action) {
        ...
        case 'googleLogin':
            ...
            if ($result['success']) {
                ...
                if (!empty($user['avatar_path'])) {
                    $user['avatar_url'] = uploadUrl('avatars', $user['avatar_path']);
                }
        ...
        case 'login':
            ...
            $user = queryOne("SELECT * FROM users WHERE email = :identifier OR phone = :identifier", [':identifier' => $identifier]);
        ...
        case 'updateUser':
            ...
            if (execute($sql, $params)) {
                $user = queryOne(
                    "SELECT id, name, email, phone, balance, isAdmin, adminType, avatar_path, registeredAt FROM users WHERE id = :id",
                    [':id' => $id]
                );
                if ($user) {
                    $user['avatar_url'] = $user['avatar_path'] ? uploadUrl('avatars', $user['avatar_path']) : '';
                }
        ...
        case 'resetPassword':
            ...
            execute("UPDATE users SET password = :password WHERE email = :email", [
                ':password' => password_hash($newPassword, PASSWORD_DEFAULT), ':email' => $email,
            ]);
            execute("DELETE FROM password_resets WHERE email = :email", [':email' => $email]);
