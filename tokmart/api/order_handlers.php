<?php
/**
 * Order placement (cash / electronic balance / bank transfer) and status
 * updates, including the cancel-refund logic for electronic payments.
 */
    ...
    switch ($action) {
        case 'addOrderWithTransfer':
            ...
            if (!execute($sql, [
                ...
                ':payment' => $payment, ':items' => json_encode($items, JSON_UNESCAPED_UNICODE), ':total' => $total,
                ':transferImage' => $transferImage, ':transferAmount' => $transferAmount,
        ...
        case 'updateOrderStatus':
            ...
            $isRefunded = (int)($order['refunded'] ?? 0);
            $payment = $order['payment'] ?? '';
            $userId = (int)$order['userId'];
            $total = (float)$order['total'];

            // Refund exactly once: only on a fresh transition into 'cancelled', only for
            // electronic payments, and only if this order hasn't already been refunded.
