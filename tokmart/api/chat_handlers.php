...
function handleChatAction(string $action): void {
    ...
    switch ($action) {
            ...
            $chats = query("
                SELECT
                    ...
                    (
                        ...
                        WHERE c2.userId = u.id AND c2.adminId = :adminId AND c2.sender = 'user' AND c2.isRead = 0
                    ) as unreadCount
                FROM users u
                LEFT JOIN (
        ...
        case 'getChatMessages':
            ...
            if ($isAdmin) {
                if ($adminId != $currentUserId) response(false, 'غير مصرح لك بعرض هذه الرسائل');
            } else {
                if ($userId != $currentUserId) response(false, 'غير مصرح لك بعرض هذه الرسائل');
            }
        ...
        case 'sendChatMessage':
            ...
            if ($isAdmin) {
                ...
                if (!$userExists) response(false, 'المستخدم غير موجود');
                if ($userExists['isAdmin'] == 1) response(false, 'لا يمكن إرسال رسائل لأدمن آخر');
            }

            if (!queryOne("SELECT id FROM users WHERE id = :id AND isAdmin = 1", [':id' => $adminId])) {
