<?php
/**
 * SMTP configuration + verification-code email sending.
 */
...
function sendEmailWithDebug(string $to, string $subject, string $htmlMessage, string $code): bool {
    ...
    $textMessage .= "هذا الكود صالح لمدة 10 دقائق.\n";
    ...
    if (class_exists('PHPMailer\PHPMailer\PHPMailer') && !empty($smtp['enabled'])) {
        try {
...
function sendVerificationEmail(string $email, string $code): bool {
    $smtp = getSMTPConfig();
    $fromName = $smtp['from_name'] ?: 'Tokmart';
    $subject = "🔐 كود التحقق الخاص بك - " . $fromName;
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                <p style='font-size: 16px; color: #333;'>مرحباً بك في <strong>{$fromName}</strong>،</p>
            ...
            <div class='footer'>
                <p>تم الإرسال من نظام التحقق الآلي - {$fromName}</p>
                <p>📧 {$smtp['from_email']}</p>
